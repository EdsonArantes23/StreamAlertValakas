...
def sync_kick_session(st: dict, kick: dict, force: bool = False) -> bool:
    ...
    if kdt is not None:
        ...
        try:
            ...
            if diff_sec <= RECONNECT_WINDOW_SEC:
                if diff_sec > 120:
                    log_line(f"Stream reconnect detect (gap: {diff_sec}s). Keeping session stats.")
                return False
        except Exception:
            reset_stream_session(st)
            st["started_at"] = kdt.isoformat()
    ...
    try:
        ...
        for temp_dir in temp_dirs:
            if os.path.exists(temp_dir):
                for pattern in ["ffmpeg-*", "*.mp4", "*.ts", "*.m3u8", "*.jpg", "*.jpeg", "*.png"]:
        ...
        if getattr(e, "errno", None) == 28:
            ...
            try:
                ...
                if getattr(e2, "errno", None) == 28:
                    notify_admin_dedup("no_space", "❌ No space left: не могу сохранить state.json.")
                    return
                raise
        raise
    finally:
    ...
    while True:
        try:
            ...
            if is_telegram_conflict_409(e):
                notify_409_dedup("⚠️ Telegram 409 Conflict (getUpdates): есть другой polling на этом токене.")
                time.sleep(10)
                continue
...
def main_loop():
    ...
    with STATE_LOCK:
        ...
        if any_live0 and not had_active_session:
            is_new_stream = True
            log_line("INIT: New stream detected (had no active session)")
    ...
    while True:
        ...
        if VK_OFFLINE_STREAK < VK_OFFLINE_STREAK_THRESHOLD and not vk.get("live"):
            ...
            vk["live"] = True
            # Берём последние известные title/category, чтобы не было ложных "обновлений"
            with STATE_LOCK:
                _st_tmp = load_state()
                vk["title"] = _st_tmp.get("vk_title")
                vk["category"] = _st_tmp.get("vk_cat")
        # ===== КОНЕЦ ФИКСА =====
...
def main():
    ...
    log_line(f"[cfg] VK_OFFLINE_STREAK_THRESHOLD={VK_OFFLINE_STREAK_THRESHOLD}")
    cleanup_temp_files()
    cleanup_old_state_backups()
    tg_drop_pending_updates_safe()
    try:
