...
HTTP_BACKOFF_MAX = float(os.getenv("HTTP_BACKOFF_MAX", "10"))
...
FFMPEG_TIMEOUT_SEC = int(os.getenv("FFMPEG_TIMEOUT_SEC", "15"))
...
def http_request_ext(method: str, url: str, *, headers=None, json_body=None, data=None, files=None, timeout=20, allow_redirects=True) -> requests.Response:
    last_exc = None
...
def http_request_tg(method: str, url: str, *, json_body=None, data=None, files=None, timeout=(5, 12)) -> requests.Response:
    last_exc = None
...
def tg_call(method: str, payload: dict, *, timeout=(5, 12)) -> dict:
...
def notify_admin(text: str) -> None:
    ...
    try:
        with STATE_LOCK:
            ...
            tg_call("sendMessage", {"chat_id": target, "text": text[:3500]}, timeout=(5, 12))
...
def tg_get_updates(offset: int, timeout: int) -> list:
    r = http_request_tg("POST", tg_api_url("getUpdates"), json_body={"offset": int(offset), "timeout": int(timeout), "allowed_updates": ["message"]}, timeout=(3, max(10, int(timeout) + 10)))
    data = r.json()
...
def tg_send_chat_action(chat_id: int, thread_id: int | None, action: str) -> None:
    try:
        ...
        tg_call("sendChatAction", payload, timeout=(3, 6))
    except Exception: pass
...
def tg_send_to(chat_id: int, thread_id: int | None, text: str, reply_to: int | None = None, with_buttons: bool = True) -> int:
    ...
    return int(tg_call("sendMessage", payload, timeout=(3, 10))["message_id"])
...
def tg_send_photo_url_to(chat_id: int, thread_id: int | None, photo_url: str, caption: str, reply_to: int | None = None) -> int:
    ...
    return int(tg_call("sendPhoto", payload, timeout=(5, 20))["message_id"])
...
def tg_send_photo_upload_to(chat_id: int, thread_id: int | None, image_bytes: bytes, caption: str, filename: str, reply_to: int | None = None) -> int:
    r = http_request_tg("POST", tg_api_url("sendPhoto"), data={"chat_id": str(chat_id), "caption": caption[:1024], "parse_mode": "HTML",
        ...
        "reply_markup": json.dumps(get_platform_keyboard())}, files={"photo": (filename, image_bytes)}, timeout=(5, 30))
    out = r.json()
    if not out.get("ok"): raise RuntimeError(f"Telegram API error: {out}")
    return int(out["result"]["message_id"])

def download_image(url: str) -> bytes:
    r = http_request_ext("GET", bust(url) or url, headers={"User-Agent": UA, "Accept": "image/avif,image/webp,image/apng,image/*,*/*;q=0.8", "Cache-Control": "no-cache", "Pragma": "no-cache"}, timeout=15)
    return r.content
...
def tg_send_to_cmd(chat_id: int, thread_id: int | None, text: str, reply_to: int | None = None) -> int:
    ...
    if reply_to is not None: payload["reply_to_message_id"] = int(reply_to)
    payload["reply_markup"] = get_platform_keyboard()
    return int(tg_call("sendMessage", payload, timeout=(3, TG_CMD_SEND_TIMEOUT_SEC))["message_id"])
...
def tg_send_photo_url_to_cmd(chat_id: int, thread_id: int | None, photo_url: str, caption: str, reply_to: int | None = None) -> int:
    ...
    if reply_to is not None: payload["reply_to_message_id"] = int(reply_to)
    payload["reply_markup"] = get_platform_keyboard()
    return int(tg_call("sendPhoto", payload, timeout=(3, TG_CMD_PHOTO_URL_TIMEOUT_SEC))["message_id"])
...
def tg_send_photo_upload_to_cmd(chat_id: int, thread_id: int | None, image_bytes: bytes, caption: str, filename: str, reply_to: int | None = None) -> int:
    r = http_request_tg("POST", tg_api_url("sendPhoto"), data={"chat_id": str(chat_id), "caption": caption[:1024], "parse_mode": "HTML",
        "message_thread_id": str(thread_id) if thread_id is not None else None, "reply_to_message_id": str(reply_to) if reply_to is not None else None,
        "reply_markup": json.dumps(get_platform_keyboard())}, files={"photo": (filename, image_bytes)}, timeout=(5, TG_CMD_PHOTO_UPLOAD_TIMEOUT_SEC))
    out = r.json()
    if not out.get("ok"): raise RuntimeError(f"Telegram API error: {out}")
    return int(out["result"]["message_id"])
...
def kick_fetch() -> dict:
    try:
        r = http_request_ext("GET", KICK_API_URL, headers=HEADERS_JSON, timeout=15)
        data = r.json()
        ...
        if isinstance(sc, dict):
            playback_url = sc.get("playback_url")
            if not playback_url: playback_url = sc.get("source") or sc.get("hls_url")
        log_line(f"Kick fetch: live={is_live}, title='{title}', cat='{cat}', viewers={viewers}, playback_url={playback_url is not None}")
...
def vk_fetch_best_effort() -> dict:
    ...
    try:
        r = http_request_ext("GET", VK_PUBLIC_URL, headers=headers, timeout=15, allow_redirects=True)
        html = r.text
    except Exception as e:
...
def send_status_with_screen_to(prefix: str, st: dict, kick: dict, vk: dict, chat_id: int, thread_id: int | None, reply_to: int | None) -> None:
    ...
    if kick.get("live") and kick.get("playback_url"):
        log_line(f"Trying screenshot from Kick: {kick['playback_url'][:80]}...")
        shot = screenshot_from_m3u8(kick["playback_url"])
        if not shot: time.sleep(2); shot = screenshot_from_m3u8(kick["playback_url"])
        if not shot: log_line("Kick screenshot failed, trying VK...")
    if not shot and vk.get("live") and vk.get("playback_url"):
        log_line(f"Trying screenshot from VK: {vk['playback_url'][:80]}...")
        shot = screenshot_from_m3u8(vk["playback_url"])
        if not shot: time.sleep(2); shot = screenshot_from_m3u8(vk["playback_url"])
    if shot:
        log_line(f"Screenshot captured: {len(shot)} bytes")
        tg_send_photo_upload_to(chat_id, thread_id, shot, caption, filename=f"live_{ts()}.jpg", reply_to=reply_to); maybe_send_to_pubg_topic(caption, st, kick); return
    log_line("No screenshot available, sending text + thumb fallback")
...
def send_caption_with_screen(caption: str, st: dict, kick: dict, vk: dict) -> None:
    ...
    if not shot and vk.get("live") and vk.get("playback_url"): shot = screenshot_from_m3u8_fresh(vk["playback_url"])
    if shot:
...
def send_status_with_screen(prefix: str, st: dict, kick: dict, vk: dict) -> None: send_status_with_screen_to(prefix, st, kick, vk, GROUP_ID, TOPIC_ID, reply_to=None)
...
def commands_loop_once():
    ...
    for upd in updates:
        ...
        try:
            ...
            cmd = text_stripped.split()[0].split("@")[0]
            ...
            if ts() - last >= START_DEDUP_SEC:
                ...
                try:
                    ...
                    with STATE_LOCK:
                        ...
                        st["last_change_sent_ts"] = ts()  # БЛОКИРУЕМ "обновление" после старта
                        st["last_platform_toggle_ts"] = ts()  # БЛОКИРУЕМ "переключение" после старта
                        save_state(st)
...
def screenshot_refresher_forever() -> None:
    while True:
        try:
            ...
            _, kick, vk, _ = snap
            if kick.get("live") and kick.get("playback_url"):
                img = screenshot_from_m3u8_fast(kick["playback_url"])
