...
def cleanup_pycache() -> None:
    try:
        ...
        for root, dirs, files in os.walk(base):
            ...
            if  " pycache " in dirs:
                try:
                    shutil.rmtree(os.path.join(root,  " pycache "), ignore_errors=True)
                except Exception:
                    pass
            for fn in files:
...
def fmt_bytes(n: int) -> str:
    ...
    if n < 1024 ** 3:
        return f"{n/10242:.1f} MB"
    return f"{n/10243:.2f} GB"

def dir_size_bytes(root: str) -> int:
    total = 0
    exclude_dirs = { " pycache ",  ".git ",  ".venv ",  "venv ",  "env ",  "node_modules "}
        ...
        if getattr(e,  "errno ", None) == 28:
            ...
            try:
                ...
                if getattr(e2,  "errno ", None) == 28:
                    notify_admin_dedup( "no_space ",  "❌ No space left: не могу сохранить state.json. Освободи место (state .json,  pycache , /tmp ffmpeg-). ")
                    return
                raise
        raise
    finally:
    ...
    try:
        ...
        for pattern in hls_patterns:
            ...
            if match:
                ...
                break
        if playback_url:
            return screensh ot_from_m3u8_fast(playback_url)
    except Exception as e:
...
def vk_fetch_best_effort() -> dict:
     " " "Parse VK Video page - checks isOnline in JSON data first. " " "
    headers = dict(HEADERS_HTML)
    headers.update({
    ...
    try:
        ...
        # Parse viewers
        viewers_matches = re.findal l(r'ViewersCounter_isLive[^ >]* >.*? <div >(\d+) </div >', html, re.IGNORECASE | re.DOTALL)
        ...
        # ===== ИСПРАВЛЕНИЕ НАЗВАНИЯ СТРИМА =====
        # 1. Ищем в data-role="markup" (точное место, где ВК хранит название стрима)
        ...
        if title_match:
            ...
            title = re.sub(r'\s+', ' ', title).strip()
            # Если название слишком длинное (>200 символов), это скорее всего мусор страницы
            if len(title) > 200:
                title = None

        # 2. Фоллбэк: если data-role не найден, пробуем BlockRenderer_markup
        if not title:
            title_match = re.search(r'BlockRenderer_markup[^ >]* >([^ <]+) </div >', html, re.IGNORECASE)
            if title_match:
                title = title_match.group(1).strip()
        
        # 3. Фоллбэк: <title> страницы (берем только если содержит ник стримера, чтобы не взять "VK Video Live")
        if not title:
            title_match = re.search(r' <title >([^ <]+) </title >', html, re.IGNORECASE)
            if title_match:
                t = title_match.group(1).strip()
                # Если в заголовке есть ник или имя канала — берем
                if VK_SLUG.lower() in t.lower() or "глад валакас" in t.lower():
                    # Режем по разделителю, если он есть (например "Глад Валакас | Название")
                    if "|" in t:
                        t = t.split("|")[-1].strip()
                    title = t
        
        # Чистим стандартный мусор
        if title:
            title = _clean_stream_title(title)
            log_line(f"VK Play: Parsed Title -> '{title}'")
        # ===== КОНЕЦ ИСПРАВЛЕНИЯ =====
...
def send_status_with_screen_to_cmd(prefix: str, st: dict, kick: dict, vk: dict, chat_id: int, thread_id: int | None, reply_to: int | None) -> None:
    ...
    if not shot and vk.get("live"):
        shot = screenshot_from_vk_page(VK_PUBLIC_URL)
    if not shot:
        cached =  shot_cache_get()
        if cached:
            shot, age = cached
    if shot:
        tg_send_photo_upload_to_cmd(chat_id, thread_id, shot, caption, filename=f "live {ts()}.jpg ", reply_to=reply_to)
        maybe_send_to_pubg_topic(caption, st, kick)
        return
    if kick.get( "live ") and kick.get( "thumb "):
        try:
...
def build_admin_diag_text(st: dict, webhook_info: dict) -> str:
    ...
    on_air = (last_poll != 0 and p oll_age  <= 120)
...
def commands_watchdog_forever():
    while True:
        try:
            ...
            with STATE_LOCK:
                st = load_state()
                last_poll = in t(st.get( "last_updates_poll_ts ") or 0)
...
def main_loop():
    ...
    while True:
        ...
        # Load previous state for comp arison
        with STATE_LOCK:
            st = load_state()
        ...
        log_line(f "POLL: Kick={kick_live}, VK={vk_live}, any={any_live} | Prev: any={prev_any}, K={prev_kick_live}, VK={prev_vk_live}, streak={prev_end_streak}, trans_streak={prev_transition_streak} ")
        ...
        # ===== SCENARIO 2: PLATFORM TOGGLE =====
            ...
            if kick_live and no t prev_kick_live:
            ...
            if changed:
                ...
                if current_ts - last  >= CHANGE_DEDUP_SEC:
                    try:
                        with STATE_LOCK:
                            st_chg = load_state()
                        caption = build_change_caption(st_c hg, kick, vk, kick_title_changed, kick_cat_changed, vk_title_changed, vk_cat_changed)
        ...
        if not any_live and prev_any and has_active_session:
            # Стрим только что выключился и была активная сессия
            time_since_live = curr ent_ts - prev_last_any_live_ts if prev_last_any_live_ts else 999999
