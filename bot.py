...
try:
    ...
    # Parse title - ИСПРАВЛЕНО: убраны лишние пробелы в шаблонах, добавлена обработка <br>
...
# Ес ли Kick скриншот не получился, пробуем VK
...
maybe_s end_to_pubg_topic(caption, st, kick)
...
with STATE_LOCK:
    ...
    if any_live0 and not had_active_sessi on:
        is_new_stream = True
    ...
    if is_new_stream or not any_live0:
        if started_at_str := st.get( "started_at "):
            try:
                started_dt = datetime.fromisoformat(started_at_str)
                age_sec = (now_utc() - started_dt).total_seconds()
                if age_s ec  > SESSION_MAX_AGE_SEC or not any_live0:
...
if BOOT_STATUS_ENABLED and any_live0:
    try:
        ...
        if can_send:
            ...
            if is_new_stream:
                send_status_with_screen( "🚨🚨 🧩 Глад Валакас запустил паток! 🚨🚨 ", st, kick0, vk0)
            else:
...
while True:
    ...
    if (not prev_any) and any_live:
        ...
        if current_ts - last  >= START_DEDUP_SEC:
            with STATE_LOCK:
                ...
                set_started_at_from_kick(st_ start, kick, force=True)
    ...
    # ===== SCENARIO 2: PLATFORM TOGGLE =====
        ...
        # (это не реально е изменение, а просто инициализация)
        if is_first_poll:
            ...
            if kic k_live and not prev_kick_live:
