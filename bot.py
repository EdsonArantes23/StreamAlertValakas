...
def sync_kick_session(st: dict, kick: dict, force: bool = False) -> bool:
    ...
    if kdt is not None:
        ...
        try:
            ...
            return True
    if force:
        st["started_at"] = kdt.isoformat() if kdt else now_utc().isoformat()
        return True
    return False
...
def default_state() -> dict:
    return {
        ...
        "startup_ping_sent": False, "kick_title": None, "kick_cat": None, "vk_title": None, "vk_cat": None,
        "kick_viewers": None, "vk_viewers": None, "last_start_sent_ts": 0, "last_change_sent_ts": 0,
        "last_boot_status_ts": 0, "last_no_stream_start_ts": 0, "updates_offset": 0,
        "last_command_seen_ts": 0, "last_commands_recover_ts": 0, "last_updates_poll_ts": 0,
        ...
        "last_quota_notify_ts": 0, "stream_stats": None, "vk_m3u8_url": None, "vk_preview_url": None
    }
...
def main_loop():
    ...
    while True:
        ...
        # === FIX: Проверка возраста started_at ДО всех проверок ===
        started_at = st.get("started_at") if isinstance(st, dict) else None
        if started_at and any_live:
            try:
                start_dt = datetime.fromisoformat(started_at)
                ...
                log_line(f"Session age check error: {e}")
        # === END FIX ===
