...
def vk_fetch_best_effort() -> dict:
    """FIXED: Parse VK Play page structure correctly based on actual HTML."""
    ...
    # Save to state for screenshots
    with STATE_LOCK:
        st = load_state()
        if thumb:
            st["vk_preview_url"] = thumb
        save_state(st)
...
def build_change_caption(st: dict, kick: dict, vk: dict, kick_title_changed: bool, kick_cat_changed: bool, vk_title_changed: bool, vk_cat_changed: bool) -> str:
    ...
    if vk.get("live"):
        ...
        if vk.get("title"):
            if vk_title_changed:
                lines.append(f"📝 <b>Название:</:** <i>{esc(vk.get('title'))}</i>")
            else:
