...
def _mask_secrets(text: str) -> str:
    ...
    try: s = re.sub(r'/bot[^/]+/', '/bot***/', s)
    except Exception: pass
    try: s = re.sub(r'bot\d+:[A-Za-z0-9_-]+', 'bot***', s)
    except Exception: pass
    return s
...
def default_state() -> dict:
    return {"any_live": False, "kick_live": False, "vk_live": False, "started_at": None, "startup_ping_sent": False,
            ...
            "last_start_sent_ts": 0, "last_change_sent_ts": 0, "last_boot_status_ts": 0, "last_no_stream_start_ts": 0,
...
def kick_fetch() -> dict:
    r = http_request_ext("GET", KICK_API_URL, headers=HEADERS_JSON, timeout=25); data = r.json()
...
def send_status_with_screen_to(prefix: str, st: dict, kick: dict, vk: dict, chat_id: int, thread_id: int | None, reply_to: int | None) -> None:
    ...
    if not shot and vk.get("live") and vk.get("playback_url"):
        ...
        if not shot: time.sleep(3); shot = screenshot_from_m3u8(vk["playback_url"])
...
def main_loop():
    ...
    with STATE_LOCK:
        st = load_state(); st["any_live"] = any_live0; st["kick_live"] = bool(kick0.get("live")); st["vk_live"] = bool(vk0.get("live"))
        if any_live0: set_started_at_from_kick(st, kick0); st["end_streak"] = 0
        st["kick_title"] = kick0.get("title"); st["kick_cat"] = kick0.get("category"); st["vk_title"] = vk0.get("title"); st["vk_cat"] = vk0.get("category")
        st["kick_viewers"] = kick0.get("viewers"); st["vk_viewers"] = vk0.get("viewers")
        stats_tick(st, kick0, vk0, any_live0, now_ts=ts()); save_state(st)
    with STATE_LOCK:
        st = load_state(); ping_sent = bool(st.get("startup_ping_sent"))
        if not ping_sent:
            try:
    ...
    if NO_STREAM_ON_START_MESSAGE and not any_live0:
        with STATE_LOCK:
            st = load_state(); last_ts = int(st.get("last_no_stream_start_ts") or 0)
    ...
    if BOOT_STATUS_ENABLED and any_live0:
        try:
            with STATE_LOCK:
                st = load_state(); can_send = ts() - int(st.get("last_boot_status_ts") or 0) >= BOOT_STATUS_DEDUP_SEC
                if can_send:
    ...
    while True:
        try: kick = kick_fetch()
        except Exception as e: kick = {"live": False, "title": None, "category": None, "viewers": None, "thumb": None, "created_at": None, "playback_url": None}; log_line(f"Kick fetch error: {e}")
        try: vk = vk_fetch_best_effort()
        except Exception as e: vk = {"live": False, "title": None, "category": None, "viewers": None, "thumb": None, "playback_url": None}; log_line(f"VK fetch error: {e}")
        with STATE_LOCK:
            st = load_state(); prev_any = bool(st.get("any_live")); prev_end_streak = int(st.get("end_streak") or 0)
            any_live = bool(kick.get("live") or vk.get("live"))
            if not prev_any and any_live:
                with STATE_LOCK:
                    st = load_state(); last = int(st.get("last_start_sent_ts") or 0)
                    if ts() - last >= START_DEDUP_SEC:
                        with STATE_LOCK: st_start = load_state(); reset_stream_session(st_start); set_started_at_from_kick(st_start, kick, force=True); save_state(st_start)
                        try:
                            with STATE_LOCK: st = load_state(); send_status_with_screen("🚨🚨 🧩 Глад Валакас запустил паток! 🚨🚨", st, kick, vk)
                            with STATE_LOCK: st = load_state(); st["last_start_sent_ts"] = ts(); save_state(st)
                        except Exception as e: log_line(f"Start send error: {e}")
            kt = kc = vt = vc = False
            with STATE_LOCK:
                st = load_state()
                if kick.get("live"): kt = kick.get("title") != st.get("kick_title"); kc = kick.get("category") != st.get("kick_cat")
                if vk.get("live"): vt = vk.get("title") != st.get("vk_title"); vc = vk.get("category") != st.get("vk_cat")
            changed = kt or kc or vt or vc
            if any_live and prev_any and changed:
                with STATE_LOCK:
                    st = load_state(); last = int(st.get("last_change_sent_ts") or 0)
                    if ts() - last >= CHANGE_DEDUP_SEC:
                        try:
                            with STATE_LOCK: st = load_state(); send_caption_with_screen(build_change_caption(st, kick, vk, kt, kc, vt, vc), st, kick, vk)
            ...
            with STATE_LOCK:
                ...
                if not any_live and (prev_end_streak + 1) >= END_CONFIRM_STREAK and cur_started and already_for != cur_started: should_send_end = True
            if should_send_end:
                try:
                    with STATE_LOCK:
                    ...
                    with STATE_LOCK: st_end2 = load_state(); st_end2["started_at"] = None; save_state(st_end2)
                except Exception as e: log_line(f"End send error: {e}")
            with STATE_LOCK:
                st = load_state(); st["any_live"] = any_live; st["kick_live"] = bool(kick.get("live")); st["vk_live"] = bool(vk.get("live"))
                if any_live: set_started_at_from_kick(st, kick); st["end_streak"] = 0
                else: st["end_streak"] = prev_end_streak + 1
                st["kick_title"] = kick.get("title"); st["kick_cat"] = kick.get("category"); st["vk_title"] = vk.get("title"); st["vk_cat"] = vk.get("category")
        ...
        if cleanup_counter >= DISK_CHECK_INTERVAL:
            ...
            with STATE_LOCK:
                stq = load_state(); last_nt = int(stq.get("last_quota_notify_ts") or 0)
...
def screenshot_refresher_forever() -> None:
    while True:
        try:
            ...
            _, kick, _, _ = snap
            if not kick.get("live") or _shot_cache_get() is not None: time.sleep(max(2, int(SHOT_REFRESH_SEC))); continue
